  ...
  try {
    await execute(
      async () => {
        ...
        setDividendosPorMes(dividendos);

        const total = Object.values(dividendos)
          .reduce((sum, mes) => sum + (mes?.total || 0), 0);

        showSuccess(`Calendario generado: ${selectedTickers.length} ticker${selectedTickers.length > 1 ? 's' : ''} - Total anual: $${total.toFixed(2)}`);
      },
      `Generando calendario de dividendos para ${selectedTickers.length} ticker${selectedTickers.length > 1 ? 's' : ''}...`
    );
  } catch (err) {
    console.error("Error al obtener dividendos:", err);
    showError(
    ...
    <BaseLayout>
      <div className="min-h-screen bg-black text-white py-8 px-4">
        <div className="max-w-4xl mx-auto bg-gray-900 p-6 rounded-2xl shadow-lg">
          <h1 className="text-3xl font-bold mb-6">📅 Calendario de Dividendos</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {months.map((month, index) => (
              ...
              >
                ...
                <p className="mb-2">
                  <strong>
                    {dividendosPorMes[index]?.total
                      ? `$${dividendosPorMes[index].total.toFixed(2)}`
                      : "Sin pagos"}
                  </strong>
                </p>
